...
int main(int argc, const char** argv)
{
	...
	std::shared_ptr<MyOutput> pshared;
	void* ptr = malloc(sizeof(pshared));
	...
	if(!mIn.data.empty()){
		...
		memcpy(ptr, mIn.data.readAccess(), sizeof(pshared));
		pshared = *static_cast<std::shared_ptr<MyOutput>*>(ptr);
	}
	else{
